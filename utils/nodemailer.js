...
//简历页面发送邮件
let sendResumeMail = (param) => {
	let mailOptions = {
			from: "简历 <thinkive@163.com>",       // 发送地址
			to: "6000329@qq.com,thinkive@126.com", // 接收列表
			cc: 'thinkive@163.com',//抄送
			bcc: '6685246@qq.com',//密送
			subject: "简历页面发送邮件地址来源："+param.email,                             // 邮件主题
			text: "简历页面发送邮件："+param.name,                          // 文本内容
			html: "内容："+param.message,
			attachments: []             
		}
	transporter.sendMail(mailOptions, function(error, response){
	    if(error){
